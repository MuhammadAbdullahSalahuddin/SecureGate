...
export default function AdminUsersPage() {
  ...
  const selfId = tokenRef.current
    ? JSON.parse(atob(tokenRef.current.split(".")[1])).userId
    : null;
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      <nav className="border-b border-zinc-800 bg-zinc-900 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-full bg-red-900/60 border border-red-800 px-2 py-0.5 text-xs font-medium text-red-300">
            {role}
          </span>
        </div>
      </nav>
      ...
      <main className="mx-auto max-w-4xl px-6 py-10 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">
              Manage operator, auditor, and admin accounts
            </p>
          </div>
          <button
            onClick={() => {
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex gap-1 flex-1">
                          {[1, 2, 3, 4, 5].map((i) => (
                            <div
                              key={i}
                              className={`h-1 flex-1 rounded-full ${i <= passed ? strengthColor : "bg-zinc-700"}`}
                            />
                          ))}
                        </div>
                        <span
                          className={`text-xs ${passed <= 2 ? "text-red-400" : passed <= 4 ? "text-yellow-400" : "text-emerald-400"}`}
                        >
                          {passed <= 2
                            ? "Weak"
                            : passed <= 4
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                        {(
                          ...
                          <div key={k} className="flex items-center gap-1.5">
                            <div
                              className={`h-1.5 w-1.5 rounded-full ${checks[k] ? "bg-emerald-500" : "bg-zinc-600"}`}
                            />
                            <span
                              className={`text-xs ${checks[k] ? "text-zinc-400" : "text-zinc-600"}`}
                            >
                              {l}
                            </span>
                          </div>
                ...
                <div>
                  ...
                  >
                    <option value="OPERATOR">
                      OPERATOR — Can request sessions
                    </option>
                    <option value="AUDITOR">
                      AUDITOR — Read-only, replay only
                    </option>
                    <option value="ADMIN">ADMIN — Full system access</option>
                  </select>
                  <p className="mt-1.5 text-xs text-zinc-500">
                    {watch("role") === "ADMIN" &&
                      "⚠ Admins can create/delete users and revoke sessions."}
                    {watch("role") === "OPERATOR" &&
                      "Can request and run terminal sessions."}
                    {watch("role") === "AUDITOR" &&
                      "Can view logs and replay sessions only."}
                  </p>
                </div>
              </div>
        ...
        <div className="flex items-start gap-3 rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3 text-xs text-zinc-500">
          <svg
            className="h-4 w-4 flex-shrink-0 mt-0.5 text-zinc-600"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          Passwords are hashed with bcrypt (12 rounds) and never stored in
          plaintext. Passwords cannot be recovered — only reset by deleting and
